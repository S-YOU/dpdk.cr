...
class L2fwd < Dpdk
  ...
  @tx_buffer = uninitialized LibDpdk::RteEthDevTxBuffer*[RTE_MAX_ETHPORTS]
  ...
  @pkts_burst = uninitialized LibDpdk::RteMbuf*[MAX_PKT_BURST]
    ...
    until @force_quit
      ...
      n_rx_port.times do |i|
        ...
        nb_rx.times do |j|
          ...
          rte_prefetch0(rte_pktmbuf_mtod(m).as(Void*))
...
", PROGRAM_NAME)
  ...
  def check_all_ports_link_status(nb_ports, port_mask)
    ...
    max_check_time.times do |count|
      ...
      nb_ports.times do |port|
        ...
        LibDpdk.rte_eth_link_get_nowait(port, pointerof(link))
  ...
  def port_init(port)
    ...
    STDOUT.flush
    @tx_buffer[port] = rte_zmalloc_socket("tx_buffer", rte_eth_tx_buffer_size(MAX_PKT_BURST), 0_u32, rte_eth_dev_socket_id(port)).as(LibDpdk::RteEthDevTxBuffer*)
