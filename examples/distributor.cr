...
class Distributor < Dpdk
  ...
  @dist_params = Pointer(LibDpdk::RteDistributor).new(0)
  @dist_tx_ring = Pointer(LibDpdk::RteRing).new(0)
  @rx_dist_ring = Pointer(LibDpdk::RteRing).new(0)
...
", PROGRAM_NAME)
  ...
  def port_init(port)
    rx_rings = 1_u16
    tx_rings = 1_u16 # lcore_count - 1 # fail on VBox
    nb_rxd, nb_txd = RX_RING_SIZE, TX_RING_SIZE

    @port_conf.rxmode.max_rx_pkt_len = ETHER_MAX_LEN
    @port_conf.rxmode.mq_mode = LibDpdk::RteEthRxMqMode::EthMqRxRss
    ...
    link = uninitialized LibDpdk::RteEthLink
    rte_eth_link_get_nowait(port, pointerof(link))
    until link.link_flags.status_up?
  ...
  def lcore_rx(arg_ptr : Void*)
    ...
    bufs = uninitialized_ptr LibDpdk::RteMbuf*[BURST_SIZE_RX]
  ...
  def flush_one_port(outbuf, outp)
    nb_tx = rte_eth_tx_burst(outp, 0, outbuf.value.mbufs, outbuf.value.count)
  ...
  def lcore_distributor(arg_ptr : Void*)
    ...
    out_r = @dist_tx_ring
    d = @dist_params
    bufs = uninitialized_ptr LibDpdk::RteMbuf*[BURST_SIZE_DIST]
  ...
  def lcore_tx(arg_ptr : Void*)
    in_r = arg_ptr.as(LibDpdk::RteRing*)
    ...
    bufs = uninitialized_ptr LibDpdk::RteMbuf*[BURST_SIZE_TX]
  ...
  def lcore_worker(arg_ptr : Void*)
    buf = uninitialized_ptr LibDpdk::RteMbuf*[8]
    d = @dist_params
