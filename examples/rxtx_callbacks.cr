...
class RxtxCallbacks < Dpdk
  ...
  @bufs = uninitialized RteMbuf*[BURST_SIZE]
  @port_conf = uninitialized RteEthConf
  @mbuf_pool = Pointer(RteMempool).new(0)
  @total_cycles = 0_u64
  @total_pkts = 0_u64

  def add_timestamps(pkts : RteMbuf**, nb_pkts : UInt16)
    now = rte_rdtsc
    nb_pkts.times do |i|
      # pkts[i].udata64 = now
  ...
  def calc_latency(pkts : RteMbuf**, nb_pkts : UInt16)
    cycles = 0_u64
    now = rte_rdtsc
  ...
  def port_init(port)
    ...
    rte_eth_add_rx_callback(port, 0, ->(port : LibDpdk::PortT, qidx : UInt16, pkts : RteMbuf**, nb_pkts : UInt16, max_pkts : UInt16, _1 : Void*) {
      RxtxCallbacks.instance.add_timestamps(pkts, nb_pkts)
    }, nil)
    rte_eth_add_tx_callback(port, 0, ->(port : LibDpdk::PortT, qidx : UInt16, pkts : RteMbuf**, nb_pkts : UInt16, _1 : Void*) {
