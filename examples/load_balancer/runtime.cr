...
class LoadBalancer < Dpdk
  ...
  def app_lcore_io_rx_buffer_to_send(lp, worker, mbuf : LibDpdk::RteMbuf*, bsz) : Void
  ...
  def app_lcore_io_rx(lp, n_workers, bsz_rd, bsz_wr, pos_lb : UInt8) : Void
    stats = uninitialized LibDpdk::RteEthStats
    lp.value.u.io.rx.n_nic_queues.times do |i|
      ...
      while j + 3 < n_mbufs
        ...
        app_lcore_io_rx_buffer_to_send(lp, worker_0, mbuf_0_0.as(LibDpdk::RteMbuf*), bsz_wr)
        app_lcore_io_rx_buffer_to_send(lp, worker_1, mbuf_0_1.as(LibDpdk::RteMbuf*), bsz_wr)
        j += 2
      end
      ...
      while j < n_mbufs
        ...
        app_lcore_io_rx_buffer_to_send(lp, worker, mbuf.as(LibDpdk::RteMbuf*), bsz_wr)
        j += 1
      end
    end
  end
  ...
  def app_lcore_io_tx_flush(lp) : Void
    lp.value.u.io.tx.n_nic_ports.times do |i|
      ...
      n_pkts = rte_eth_tx_burst( port, 0, mbuf_out_ptr.value.array, mbuf_out_ptr.value.n_mbufs.to_u16)
  ...
  def app_lcore_worker(lp, bsz_rd, bsz_wr) : Void
    lp.value.u.worker.n_rings_in.times do |i|
      ...
      bsz_rd.times do |j|
        ...
        pkt = mbuf_in_array[j].as(LibDpdk::RteMbuf*)
        ipv4_hdr = rte_pktmbuf_mtod_offset(pkt, sizeof(LibDpdk::EtherHdr)).as(LibDpdk::Ipv4Hdr*)
  ...
  def app_lcore_worker_flush(lp) : Void
    APP_MAX_NIC_PORTS.times do |port|
      ...
      if likely(mbuf_out_flush_ptr.value == 0 ||
        mbuf_out_ptr.value.n_mbufs == 0)
        mbuf_out_flush_ptr.value = 1_u8
        next
      end
