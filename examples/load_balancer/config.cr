...
At least one RX port is not enabled for TX.\n")
  ...
  def app_print_params
    ...
    @app.n_lpm_rules.times do |rule|
      ip = @app.lpm_rules.pointer_at(rule).value.ip
      depth = @app.lpm_rules.pointer_at(rule).value.depth
      if_out = @app.lpm_rules.pointer_at(rule).value.if_out
  ...
  private def pton(address) : UInt32
    ...
    return bswap32(addr.s_addr) if LibC.inet_pton(LibC::AF_INET, address, pointerof(addr)) == 1
    0_u32
  end
end
