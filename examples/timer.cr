...
class Timer < Dpdk
  ...
  @timer0 = uninitialized LibDpdk::RteTimer
  @timer1 = uninitialized LibDpdk::RteTimer
  ...
  def timer1_cb(tim)
    ...
    rte_timer_reset(tim, hz/3, LibDpdk::RteTimerType::Single, lcore_id, ->(tim : LibDpdk::RteTimer*, arg : Void*) {
  ...
  def main(argc, argv)
    ...
    hz = rte_get_timer_hz
    lcore_id = rte_lcore_id
    rte_timer_reset(pointerof(@timer0), hz, LibDpdk::RteTimerType::Periodical, lcore_id, ->(tim : LibDpdk::RteTimer*, arg : Void*) {
      Timer.instance.timer0_cb(tim)
    }, nil)

    # load timer1, every second/3, on next lcore, reloaded manually
    lcore_id = rte_get_next_lcore(lcore_id, 0, 1)
    rte_timer_reset(pointerof(@timer1), hz/3, LibDpdk::RteTimerType::Single, lcore_id, ->(tim : LibDpdk::RteTimer*, arg : Void*) {
      Timer.instance.timer1_cb(tim)
    }, nil)
