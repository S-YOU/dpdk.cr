...
", PROGRAM_NAME)
  ...
  def flush_tx_error_callback(unsent : LibDpdk::RteMbuf**, count : UInt16, _userdata : Void*)
  ...
  def free_tx_buffers(tx_buffer : LibDpdk::RteEthDevTxBuffer**)
  ...
  def configure_tx_buffers(tx_buffer : LibDpdk::RteEthDevTxBuffer**)
    nb_ports = rte_eth_dev_count
    nb_ports.times do |port_id|
      next if (@portmask & (1 << port_id)) == 0

      # Initialize TX buffers
      tx_buffer[port_id] = rte_zmalloc_socket("tx_buffer", rte_eth_tx_buffer_size(MAX_PKTS_BURST), 0, rte_eth_dev_socket_id(port_id)).as(LibDpdk::RteEthDevTxBuffer*)

      rte_eth_tx_buffer_init(tx_buffer[port_id], MAX_PKTS_BURST)

      ret = rte_eth_tx_buffer_set_err_callback(tx_buffer[port_id], ->(unsent : LibDpdk::RteMbuf**, count : UInt16, userdata : Void*) {
  ...
  def print_stats
    eth_stats = uninitialized LibDpdk::RteEthStats
    nb_ports = rte_eth_dev_count
  ...
  # ring.
  def rx_thread(ring_out : LibDpdk::RteRing*) : Void
    recv_pkts = uninitialized_ptr LibDpdk::RteMbuf*[MAX_PKTS_BURST]
    nb_ports = rte_eth_dev_count
    seqn = 0_u32

    rte_log(INFO, REORDERAPP, "rx_thread() started on lcore %u\n", rte_lcore_id)

    portmask, rx_pkts_ptr, enqueue_pkts_ptr, enqueue_failed_pkts_ptr =
      @portmask, pointerof(@app_stats.@rx.@rx_pkts), pointerof(@app_stats.@rx.@enqueue_pkts), pointerof(@app_stats.@rx.@enqueue_failed_pkts)
  ...
  def worker_thread(arg_ptr : Void*) : Void
    burst_buffer = uninitialized_ptr LibDpdk::RteMbuf*[MAX_PKTS_BURST]
    nb_ports = rte_eth_dev_count
    burst_size = 0_u16
    ...
    until @quit_signal
      ...
      burst_size.times do |i|
        # burst_buffer[i].value.port ^= xor_val
  ...
  def send_thread(arg_ptr : Void*) : Void
    ...
    send_mbufs = uninitialized_ptr LibDpdk::RteMbuf*[MAX_PKTS_BURST]
    send_rombufs = uninitialized_ptr LibDpdk::RteMbuf*[MAX_PKTS_BURST]
    send_tx_buffer = uninitialized_ptr LibDpdk::RteEthDevTxBuffer*[RTE_MAX_ETHPORTS]
    ...
    until @quit_signal
      ...
      nb_dq_mbufs.times do |i|
        ...
        if ret == -1 && errno == LibC::ERANGE
          ...
          if rte_eth_tx_burst(outp, 0_u16, send_mbufs[i].as(LibDpdk::RteMbuf**), 1_u16) != 1_u16
            rte_pktmbuf_free(send_mbufs[i])
  ...
  def tx_thread(arg_ptr : Void*) : Void
    ring_in = arg_ptr.as(LibDpdk::RteRing*)
    tx_mbufs = uninitialized_ptr LibDpdk::RteMbuf*[MAX_PKTS_BURST]
    tx_buffer = uninitialized_ptr LibDpdk::RteEthDevTxBuffer*[RTE_MAX_ETHPORTS]
    rte_log(INFO, REORDERAPP, "tx_thread() started on lcore %u\n", rte_lcore_id)

    configure_tx_buffers(tx_buffer)

    portmask, dequeue_pkts_ptr, ro_tx_pkts_ptr = @portmask, pointerof(@app_stats.@tx.@dequeue_pkts),
      pointerof(@app_stats.@tx.@ro_tx_pkts)
