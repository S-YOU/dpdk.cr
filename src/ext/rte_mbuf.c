 ...
 *   - NULL if allocation failed.
 */
extern inline struct rte_mbuf *rte_pktmbuf_alloc(struct rte_mempool *mp)
{
	struct rte_mbuf *m;
 ...
 */
extern inline struct rte_mbuf *rte_pktmbuf_clone(struct rte_mbuf *md,
 ...
 */
extern inline char *rte_pktmbuf_prepend(struct rte_mbuf *m,
					uint16_t len)
{
	__rte_mbuf_sanity_check(m, 1);
 ...
 */
extern inline char *rte_pktmbuf_append(struct rte_mbuf *m, uint16_t len)
{
	void *tail;
 ...
 */
extern inline char *rte_pktmbuf_adj(struct rte_mbuf *m, uint16_t len)
{
	__rte_mbuf_sanity_check(m, 1);
 ...
 *   - -1: On error.
 */
extern inline int rte_pktmbuf_trim(struct rte_mbuf *m, uint16_t len)
{
	struct rte_mbuf *m_last;
